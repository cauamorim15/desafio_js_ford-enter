...
function SetCarToCompare(el, carClass) {
        ...
        if(el.checked && carArr.length >=2){
               alert('Só pode marcar 2 carros ao mesmo tempo!') ;
                el.checked = false;
                return;
        }
        if(el.checked){
...
function UpdateCompareTable() {
   ...
   document.querySelector("#compare_preco_0").innerHTML = (compare0.preco / 1).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
    document.querySelector("#compare_preco_1").innerHTML = (compare1.preco / 1).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
